...
const CustomLink = ({href, title, className=""}) => {
  ...
  return(
    <Link href={href} className={`${className} relative group`}>
      ...
      <span className={
        `h-[1px] inline-block bg-dark absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease duration-300 dark:bg-light ${router.asPath === href ? 'w-full' : 'w-0'}`
        }>&nbsp;</span>
    </Link>
  )
}
...
const CustomMobileLink = ({href, title, className="", toggle}) => {
  ...
  return(
    <button href={href} className={`${className} relative group text-light dark:text-dark my-2`} onClick={handleClick}>
      ...
      <span className={
        `h-[1px] inline-block bg-light absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease duration-300 dark:bg-dark ${router.asPath === href ? 'w-full' : 'w-0'}`
        }>&nbsp;</span>
    </button>
  )
}
  ...
  return (
    ...
    >
     ...
     <nav>
      ...
      <nav className="flex items-center justify-center flex-wrap">
        <motion.a href="https://github.com/Mathouzalem" target={"_blank"}
        whileHover={{y: -2}}
        whileTape={{scale:0.9}}
        className="w-6 mr-3">
          <GithubIcon />
        </motion.a>
        <motion.a href="https://www.linkedin.com/in/mathilde-bonhomme-ab3abb12b/" target={"_blank"}
        whileHover={{y: -2}}
        whileTape={{scale:0.9}}
...

    {
     ...
     <nav className='flex items-center flex-col justify-center'>
      ...
      <button
      onClick={() => setMode(mode === "light" ? "dark" : "light")}
      className={`ml-3 flex items-center justify-center rounded-full p-1 w-6 h-6 ${mode === "light" ? "bg-dark text-light" : "bg-light text-dark"}`}
      >
        {
          mode === "dark" ?
          <SunIcon className={"fill-dark"}/>
